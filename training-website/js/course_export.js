  ...
  if (!data) {
    root.innerHTML = `
      <div class="export-document" style="text-align: center;">
        ...
        <p><a href="${escapeHtml(window.courseAssetUrl("index.html"))}">← Back to Course</a></p>
      </div>
    `;
    return;
  }
    ...
    const slidesHtml = module.slides
      .map((slide) => {
        ...
        const diagramHtml = slide.visualization
          ? `<div class="export-diagram"><img src="${escapeHtml(window.courseAssetUrl(`visualizations/${slide.visualization}`))}" alt="${escapeHtml(slide.title)} diagram" decoding="async"></div>`
          : "";
