...
function initCourseHome() {
  ...
  if (ctaBtn) {
    if (completedCount > 0) {
      ...
      // Resume the first unfinished step, including quiz questions and answers.
      let targetSlideId = null;
      for (const module of courseData.modules) {
        const stepIds = module.slides.map(slide => slide.id);
        for (const question of module.quiz || []) {
          stepIds.push(`q-${module.id}.${question.id}`, `q-${module.id}.${question.id}-solved`);
        }
        for (const stepId of stepIds) {
          if (!completedSlides.includes(stepId)) {
            targetSlideId = stepId;
            break;
          }
        }
      ...
      if (targetSlideId) {
        ctaBtn.href = window.courseAssetUrl(`viewer.html?slide=${targetSlideId}`);
      }
      ...
      if (courseData.modules[0] && courseData.modules[0].slides[0]) {
        ctaBtn.href = window.courseAssetUrl(`viewer.html?slide=${courseData.modules[0].slides[0].id}`);
      }
  ...
  courseData.modules.forEach(module => {
    ...
    card.innerHTML = `
      ...
      <div style="display: flex; gap: 12px; margin-top: 12px; border-top: 1px solid var(--border-main); padding-top: 16px; flex-wrap: wrap;">
        <a href="${window.escapeHtml(window.courseAssetUrl(`viewer.html?slide=${targetSlideId}`))}" class="btn btn-secondary" style="font-size: 13px; padding: 8px 16px;">
          View Slides
        </a>
        ${module.quiz && module.quiz.length > 0 ? `
          <a href="${window.escapeHtml(window.courseAssetUrl(`viewer.html?quiz=${displayNum(module.id)}`))}" class="btn btn-secondary" style="font-size: 13px; padding: 8px 16px; border-color: rgba(16, 185, 129, 0.3); color: var(--success); background-color: rgba(16, 185, 129, 0.03);">
