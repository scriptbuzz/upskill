...
function renderSidebar() {
  ...
  AIF_COURSE_DATA.modules.forEach(module => {
    ...
    modHeader.innerHTML = `
      <span>Module ${module.id.replace('m', '')}: ${module.title}</span>
    ...
    module.slides.forEach(slide => {
      ...
      slideLi.id = `sidebar-item-${slide.id.replace('.', '_')}`;
      slideLi.setAttribute("data-slide-id", slide.id);
      
      const isCompleted = completedSlides.includes(slide.id);
      slideLi.innerHTML = `
        <span class="slide-title-text">${slide.id.replace('m', '').replace('-s', '.')} ${slide.title}</span>
    ...
    if (module.quiz && module.quiz.length > 0) {
      ...
      module.quiz.forEach(question => {
        ...
        qLi.id = `sidebar-item-q-${module.id}_${question.id}`;
        ...
        qSolvedLi.id = `sidebar-item-q-${module.id}_${question.id}-solved`;
...
function updateSidebarProgress() {
  AIF_COURSE_DATA.modules.forEach(module => {
    ...
    module.slides.forEach(slide => {
      const escapedId = slide.id.replace('.', '_');
      const check = document.querySelector(`#sidebar-item-${escapedId} .slide-bullet-check`);
      if (check) {
        const isCompleted = completedSlides.includes(slide.id);
    ...
    if (module.quiz) {
      module.quiz.forEach(q => {
        // Unsolved question
        const checkQ = document.querySelector(`#sidebar-item-q-${module.id}_${q.id} .slide-bullet-check`);
        if (checkQ) {
          const isCompleted = completedSlides.includes(`q-${module.id}.${q.id}`);
          checkQ.style.display = isCompleted ? "inline" : "none";
        }
        // Solved question
        const checkQS = document.querySelector(`#sidebar-item-q-${module.id}_${q.id}-solved .slide-bullet-check`);
        if (checkQS) {
...
function loadInitialContent() {
  ...
  if (slideParam) {
    ...
    const modId = quizParam.startsWith("m") ? quizParam : `m${quizParam}`;
...
function navigateSlide(index) {
  ...
  if (item.type === "slide") {
    ...
    const escapedId = item.id.replace('.', '_');
    const sidebarItem = document.getElementById(`sidebar-item-${escapedId}`);
    if (sidebarItem) {
    ...
    const cleanModuleNum = item.moduleId.replace('m', '');
    const cleanSlideNum = item.id.replace('m', '').replace('-s', '.');
    document.getElementById("active-slide-title").innerText = `Module ${cleanModuleNum} Outline — Slide ${cleanSlideNum}`;
    ...
    const escapedQId = item.id.replace('.', '_');
    const sidebarQuiz = document.getElementById(`sidebar-item-${escapedQId}`);
    if (sidebarQuiz) {
