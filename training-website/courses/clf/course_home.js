...
function initCourseHome() {
  ...
  if (ctaBtn) {
    if (completedCount > 0) {
      ...
      for (const module of CLF_COURSE_DATA.modules) {
      ...
      if (!targetSlideId && CLF_COURSE_DATA.modules[0] && CLF_COURSE_DATA.modules[0].slides[0]) {
        targetSlideId = CLF_COURSE_DATA.modules[0].slides[0].id;
      }
      if (targetSlideId) {
      ...
      if (CLF_COURSE_DATA.modules[0] && CLF_COURSE_DATA.modules[0].slides[0]) {
        ctaBtn.href = `viewer.html?slide=${CLF_COURSE_DATA.modules[0].slides[0].id}`;
      }
  ...
  CLF_COURSE_DATA.modules.forEach(module => {
    ...
    card.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 12px;">
        <div>
          <span class="badge badge-muted" style="margin-bottom: 8px;">Module ${module.id}</span>
      ...
      <div style="display: flex; gap: 12px; margin-top: 12px; border-top: 1px solid var(--border-main); padding-top: 16px; flex-wrap: wrap;">
        ...
        ${module.quiz && module.quiz.length > 0 ? `
          <a href="viewer.html?quiz=${module.id}" class="btn btn-secondary" style="font-size: 13px; padding: 8px 16px; border-color: rgba(16, 185, 129, 0.3); color: var(--success); background-color: rgba(16, 185, 129, 0.03);">
